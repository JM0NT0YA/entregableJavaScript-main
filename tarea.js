...
const calculateAverage = (array) => {
    if (array.length === 0) return NaN; //  Devuelve NaN para un array vacío
    const validNumbers = array.filter(number => !isNaN(number)); // Filtra los números válidos
    if (validNumbers.length === 0) return NaN; // Devuelve NaN si no hay números válidos
    const sum = validNumbers.reduce((total, number) => total + number, 0);
  ...
  const calcularDuracionTotal = (peliculas) => {
    return peliculas.reduce((total, pelicula) => {
        // Ignora las películas con duración negativa
        return pelicula.duracion >= 0 ? total + pelicula.duracion : total;
    }, 0);
};
  ...
  const buscarPeliculas = (peliculas, titulo, genero) => {
    if (!genero) return peliculas.filter(pelicula => pelicula.titulo.toLowerCase().includes(titulo.toLowerCase()));
  ...
  const calcularPromedioPuntajes = (peliculas) => {
    const validPeliculas = peliculas.filter(pelicula => pelicula.puntaje && pelicula.puntaje >= 0);
    if (validPeliculas.length === 0) return 0;
    const sum = validPeliculas.reduce((total, pelicula) => total + pelicula.puntaje, 0);
    return sum / validPeliculas.length;
  };
  ...
  const filtrarPorAño = (peliculas, año) => {
    if (!año) return []; // Retorna un array vacío si no se proporciona un año
  ...
  const calcularPromedioDuracionPorGenero = (peliculas, genero) => {
    const validPeliculas = peliculas.filter(pelicula => pelicula.genero && pelicula.genero.toLowerCase() === genero.toLowerCase() && pelicula.duracion && pelicula.duracion > 0);
    if (validPeliculas.length === 0) return 0;
    const sum = validPeliculas.reduce((total, pelicula) => total + pelicula.duracion, 0);
    return sum / validPeliculas.length;
  };
  ...
  // Otras funciones y clases no modificadas...
